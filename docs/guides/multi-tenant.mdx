...
<AccordionGroup>
  ...
  <Accordion title="A provisioning attempt that never settled">
    ...
    its lease. If a claim is released after the lease lapses while AgentPhone is
    still answering, the late response still binds the tenant key so a retry
    cannot provision a second sub-account.
  </Accordion>
