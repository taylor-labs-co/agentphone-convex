...
<AccordionGroup>
  ...
  <Accordion title="A provisioning attempt that never finished">
    If AgentPhone timed out or returned a `5xx`, the claim stays in place and
    later attempts throw rather than risk a duplicate. Run `syncSubAccounts`,
    then either `adoptSubAccount` (AgentPhone did create it) or
    `releaseSubAccountClaim` (it did not).
  </Accordion>
